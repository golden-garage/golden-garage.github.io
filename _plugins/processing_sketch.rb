...
module Jekyll
  class PSketch < Liquid::Tag
    ...
    def render(context)
      ...
      source += "<canvas id=\"#{@sketchid}\" data-processing-sources=\"#{context.registers[:site].config['baseurl']}/assets/#{@path}\" width=\"#{@width}\" height=\"#{@height}\">\n"
      source += "<figure>\n"
      source += "<p>Your browser does not support the canvas tag.</p>\n</canvas>\n"
      source += "<figcaption>View source code: <a href=\"#{context.registers[:site].config['baseurl']}/assets/#{@path}\">#{srcname}</a></figcaption></figure>\n"
      source
    end
  end
end
